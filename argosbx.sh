...
cat > $HOME/agsbx/watchdog.sh <<'EOF'
#!/bin/sh
while true; do
  if ! pgrep -f 'agsbx/sing-box' >/dev/null 2>&1; then
    nohup $HOME/agsbx/sing-box run -c $HOME/agsbx/sb.json >/dev/null 2>&1 &
  fi
  ...
  if [ -f "$HOME/agsbx/argo.log" ] && ! pgrep -f 'agsbx/cloudflared' >/dev/null 2>&1; then
    nohup $HOME/agsbx/cloudflared tunnel --url http://localhost:$(cat $HOME/agsbx/port_vm_ws) --edge-ip-version auto --no-autoupdate --protocol http2 > $HOME/agsbx/argo.log 2>&1 &
  fi
...
nohup $HOME/agsbx/watchdog.sh >/dev/null 2>&1 &
else
